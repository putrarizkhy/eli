...
class EbooksController extends AppController {
	...
	function _uploadcover($output_dir){
		$status = array('status'=>FALSE,'message'=>'File tidak ditemukan','filename'=>'');
		if($this->data['Ebook']['ebookcover'])
		{
		    //$output_dir = "uploads/";
		    //ini_set("display_errors",1);
		    if(isset($this->data['Ebook']['ebookcover']))
		    {
		        //$RandomNum   = time();
		        
		        $ImageName      = str_replace(' ','-',strtolower($this->data['Ebook']['ebookcover']['name']));
		        $ImageType      = $this->data['Ebook']['ebookcover']['type']; //"image/png", image/jpeg etc.
	            ...
	            move_uploaded_file($this->data['Ebook']['ebookcover']["tmp_name"],$output_dir. $NewImageName);
	...
	function _uploadstaticpdf($output_dir){
		$status = array('status'=>FALSE,'message'=>'File tidak ditemukan','filename'=>'');
		if($this->data['Ebook']['ebookstatic'])
		{
		    //$output_dir = "uploads/";
		    //ini_set("display_errors",1);
		    if(isset($this->data['Ebook']['ebookstatic']))
		    {
		        //$RandomNum   = time();
		        
		        $ImageName      = str_replace(' ','-',strtolower($this->data['Ebook']['ebookstatic']['name']));
		        $ImageType      = $this->data['Ebook']['ebookstatic']['type']; //"image/png", image/jpeg etc.
		        ...
		        else
		        {
		            ...
		            move_uploaded_file($this->data['Ebook']['ebookstatic']["tmp_name"],$output_dir. $NewImageName);
...
<Settings
	...
	CopyrightMessage=""
...
<Language>
	...
	function admin_add($id) {
		...
		$sequence = $this->Ebook->find('first',array('conditions'=>array('Sequence.pustaka'=>'ebook')));
		...
		if (!empty($this->data)) {
			...
			$this->data['Ebook']['id_pustaka']= $lastidtoinsert_id2;

			$tipeebook =  $this->data['Ebook']['type'];
			...
			if ($this->Ebook->save($this->data)) {
				...
				if($tipeebook == 1){
					...
					$pdf_ok = $this->_uploadstaticpdf($newdirectory.DS);
					$cover_ok = $this->_uploadcover($newdirectory.DS);

					$coverpath = 'files/ebooks/'.$lastID.'/'.$cover_ok['filename'];

					$pdfpath = 'files/ebooks/'.$lastID.'/'.$pdf_ok['filename'];

					$this->Ebook->saveField('cover', $coverpath);
					$this->Ebook->saveField('pdffile', $pdfpath);
					$this->Ebook->saveField('jumlahhalaman', 200);

				}
				
				$sequence = $this->Ebook->Sequence->read(null, $idsequence);
				$current=$sequence['Sequence']['value']+1;
				$this->Ebook->Sequence->saveField('value',$current);
