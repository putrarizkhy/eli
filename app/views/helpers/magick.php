<?php  
class MagickHelper extends Helper { 
    ...
    */ 
    ...
    function pdfconvert($filePath,$pdfFilePath, $width, $height, $tag=true, $cachePath=null, $quality=100, $aspect=true, $options = array()) {
        $fullpath = WWW_ROOT; 
        if(!$cachePath) $cachePath = $this->cachePath; 
        $url = $fullpath.$filePath; 
        $urlpdf = $fullpath.$pdfFilePath;
        ...
        $relfile ='_'.basename($filePath); 
        //$relfile2 = $this->webroot.$this->themeWeb.$cachePath.'/'.$width.'x'.$height.'_'.basename($filePath); 
        ...
        { 
            ...
            exec($this->convertPath.' "'.$pdfFilePath.'[0]" -colorspace RGB -geometry 200 '.escapeshellarg($cachefile).'');
    ...
    function resize($filePath, $width, $height, $tag=true, $cachePath=null, $quality=100, $aspect=true, $options = array()) {
        ...
        // Relative file (for return use) 
        $relfile = $width.'x'.$height.'_'.basename($filePath); 
        //$relfile2 = $this->webroot.$this->themeWeb.$cachePath.'/'.$width.'x'.$height.'_'.basename($filePath); 
        // Location on server (for resize use) 
        $cachefile = $fullpath.$cachePath.DS.$width.'x'.$height.'_'.basename($filePath); 
