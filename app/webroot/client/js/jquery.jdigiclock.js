...
(function($) {
    $.fn.extend({
        ...
        jdigiclock: function(options) {
            ...
            var defaults = {
                ...
                defaultWeatherImage : 'images/weather/1.png',
                defaultCity : 'Jakarta',
                lang: 'en',
                am_pm: false,
                weatherLocationCode: 'ASI|ID|ID009|JAKARTA',
                weatherMetric: 'C',
                weatherUpdate: 0,
                proxyType: 'php'
    ...
    $.fn.getWeather = function(el) {
        ...
        $.ajax({
          url: 'js/proxy/' + proxy + '?location=' + el.weatherLocationCode + '&metric=' + el.weatherMetric,
          dataType: 'json',
          ...
          error: function(data) {
            ...
            el.find('#weather').css('background','url(' + defaultWeatherImage+') 50% 100% no-repeat');
            var weather = '<div id="local"><p class="city">' + defaultCity + '</p></div>';
            weather += '<div id="temp"><p id="date">' + el.currDate + '</p></div>';
            el.find('#weather').html(weather);
        ...
        /*$.getJSON('js/proxy/' + proxy + '?location=' + el.weatherLocationCode + '&metric=' + el.weatherMetric, function(data) {
